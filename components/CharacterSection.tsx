    ...
    <div className="space-y-4">
      ...
      {viewingCharacter && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            {/* Image Header */}
            <div className="w-full bg-slate-100 dark:bg-slate-900 relative h-56 md:h-72 shrink-0">
              {viewingCharacter.imageUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <User className="w-20 h-20 text-slate-300 dark:text-slate-700" />
                </div>
              )}
              <div className="absolute top-3 right-3 flex gap-2">
                <button 
                  onClick={() => setViewingCharacter(null)}
                  className="p-1.5 bg-white/90 dark:bg-slate-800/90 backdrop-blur rounded-lg text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white shadow-lg transition-all"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="p-5 flex flex-col overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white leading-tight">{viewingCharacter.name}</h3>
              </div>
              
              <div className="flex-1">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Информация</h4>
                <div className="text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-wrap text-base">
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="p-2.5 text-red-500 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-all"
                  title="Удалить"
                >
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Персонажи</h2>
        <button
          onClick={() => setIsAdding(true)}
          className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all font-medium shadow-md shadow-indigo-100 dark:shadow-indigo-900/20"
        >
      ...
      {(isAdding || editingId) && (
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl p-5 animate-in zoom-in duration-200 overflow-y-auto max-h-[95vh] custom-scrollbar">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">
                {editingId ? 'Редактировать героя' : 'Новый персонаж'}
              </h3>
              <button onClick={resetForm} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md transition-all">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex-1 flex items-center justify-center bg-slate-50 dark:bg-slate-900 rounded-lg overflow-hidden border-2 border-dashed border-slate-200 dark:border-slate-700 min-h-[180px]">
                  {formData.imageUrl ? (
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg focus:ring-4 focus:ring-indigo-500/10 dark:focus:ring-indigo-900/20 focus:border-indigo-500 outline-none h-40 resize-none leading-relaxed transition-all"
            ...
            <div className="flex gap-2 mt-10">
              ...
              <button
                onClick={handleSave}
                className="flex-1 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 transition-all "
              >
                Сохранить
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {project.characters.map((char) => (
          <div 
            ...
            className="group bg-white dark:bg-slate-800 rounded-lg overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-xl dark:hover:shadow-xl hover:border-indigo-200 dark:hover:border-indigo-800 transition-all cursor-pointer flex flex-col h-full"
          >
            <div className="h-56 bg-slate-100 dark:bg-slate-900 relative overflow-hidden">
              ...
              <div className="absolute top-3 right-3 flex gap-2 sm:opacity-0 group-hover:opacity-100 transition-all">
                <button
                  ...
                  className="p-2.5 bg-white/95 dark:bg-slate-800/95 backdrop-blur rounded-lg text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 shadow-md transition-colors"
                  title="Редактировать"
                >
                  <Edit3 className="w-4 h-4" />
                </button>
                <button
                  ...
                  className="p-2.5 bg-white/95 dark:bg-slate-800/95 backdrop-blur rounded-lg text-slate-600 dark:text-slate-300 hover:text-red-500 dark:hover:text-red-400 shadow-md transition-colors"
                  title="Удалить"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
            ...
            <div className="p-6 flex-1 flex flex-col">
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{char.name}</h3>
      ...
      {project.characters.length === 0 && !isAdding && (
        <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
          <div className="bg-slate-50 dark:bg-slate-900 w-16 h-16 rounded-md flex items-center justify-center mx-auto mb-4">
